...
def freeze_index(es, source_index_spec, delete_source, newindex=False, tag=False):
    ...
    for index in indices:
        ...
        if tag:
            ...
            es.indices.put_settings(index=index, body='{ "index": { "blocks.read_only": true } }')
            ...
            es.indices.put_settings(index=index, body='{ "index": { "blocks.read_only": null } }')
            print('- Return clone index to read-write')
            es.indices.put_settings(index=frozen_index, body='{ "index": { "blocks.read_only": null } }')
...
parser = argparse.ArgumentParser(description='Disable or re-enable Elasticsearch indices, optionally renaming the frozen index.')
parser.add_argument('-e', '--host', dest='host', default='127.0.0.1', help='Elasticsearch IP address')
parser.add_argument('-p', '--port', dest='port', default='9200', help='Elasticsearch port')
parser.add_argument('-a', '--action', dest='action', help='Action to take.', choices=['freeze', 'thaw', 'list'], required=True)
parser.add_argument('-i', '--index', dest='index', help='Index to act on.')
parser.add_argument('-t', '--tag', dest='tag', default=False, help='An optional tag to add to indices being frozen.  E.g. the "httpdlog-2021.10" index becomes "httpdlog-lab3.1-2021.10" by specifying a tag of "lab3.1".  Only works for the freeze action.')
parser.add_argument('-n', '--newindex', dest='newindex', default=False, help='Name for the new index to be created.  Overrides -t/--tag.')
parser.add_argument('-d', '--delete', dest='delete', action='store_true', help='Delete the source index for a "freeze" action.  Only works when -t or -n are specified.')
...
if args.action == 'list':
    ...
    if len(populated_indices) == 0:
        ...
        for index in populated_indices:
            res = es.count(index='%s' % (index), body={'query': {'match_all': {}}}, ignore_unavailable=True)
