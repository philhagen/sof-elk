...
def process_cloudtrail_file(infile, outfh):
    ...
    if not "Records" in rawjson.keys():
        sys.stderr.write(
            "- ERROR: Input file %s does not appear to contain AWS Cloudtrail records. Skipping file.\n"
            % (infile)
        )
        return
...
parser = argparse.ArgumentParser(
    description="Process AWS Cloudtrail logs into a format that SOF-ELK(R) can read, in ndjson form."
)
parser.add_argument(
    "-r",
    "--read",
    dest="infile",
    help="AWS Cloudtrail log file to read, or a directory containing AWS Cloudtrail log files.  Files can be in native JSON or gzipped JSON.",
)
parser.add_argument(
    "-w",
    "--write",
    dest="outfile",
    help="File to create containing processed AWS Cloudtrail data.",
)
parser.add_argument(
    "-f",
    "--force",
...
if args.outfile == None:
    ...
    sys.stderr.write(
        'ERROR: Output file does not end with ".json".  SOF-ELK requires this extension to process these logs.  Exiting.\n'
    )
    sys.exit(2)
...
if not args.outfile.startswith(default_destdir):
    print(
        "You must move/copy the generated file to the %s directory before SOF-ELK can process it."
        % (default_destdir)
    )
else:
    print(
