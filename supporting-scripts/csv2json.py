...
def remove_empty_fields(obj):
    ...
    if isinstance(obj, dict):
        return {key: remove_empty_fields(value) for key, value in obj.items() if value != ''}
    elif isinstance(obj, list):
        return [remove_empty_fields(item) for item in obj if item is not None and item != '']
    else:
        return obj
...
if __name__ == "__main__":
    ...
    parser.add_argument('-w', '--write', dest='outfile', help='JSON output file to create', required=True)
    parser.add_argument('-t', '--tag', dest='tags', help='Optional string to add to "tags" field - can be used multiple times', action='append')
