...
* Logstash will ingest all files from the following filesystem locations:
  ...
  * ```/logstash/kape/```: Files generated by the [KAPE](https://learn.duffandphelps.com/kape) triage collection tool ([See this document](doc/kape_support.md) for details on which specific modules are currently supported)
