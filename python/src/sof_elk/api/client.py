...
from sof_elk.api.custom_exceptions import SOFElkConnectionError, SOFElkTimeOutError, SOFElkResponseError
...
class SOFElkSession:
    ...
    """
    @staticmethod
    def get_session(retries: int = 3, backoff_factor: float = 0.3, status_forcelist: Tuple[int, ...] = (500, 502, 503, 504)) -> 'SOFElkHTTPClient':
        """
...
class SOFElkHTTPClient:
    ...
    def request(self, method: str, url: str, **kwargs: Any) -> requests.Response:
        ...
        try:
            ...
            try:
                ...
                raise SOFElkResponseError(f"HTTP Error: {e}", status_code=response.status_code, response_body=response.text)
            return response
        except requests.exceptions.Timeout:
            raise SOFElkTimeOutError(f"Request timed out contacting {url}")
        except requests.exceptions.ConnectionError:
            raise SOFElkConnectionError(f"Failed to connect to {url}")
        except requests.exceptions.RequestException as e:
            raise SOFElkResponseError(f"An unexpected error occurred: {e}")
    ...
    def download_file(self, url: str, target_path: str, chunk_size: int = 8192) -> bool:
        ...
        try:
            ...
            raise SOFElkResponseError(f"Failed to download {url}: {e}")
