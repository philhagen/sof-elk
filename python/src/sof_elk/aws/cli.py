...
def register_subcommand(parser: argparse.ArgumentParser) -> None:
    ...
    ct_parser.add_argument(
        "-r", "--read", dest="input", required=True,
        help="AWS CloudTrail log file or directory to read."
    )
    ct_parser.add_argument(
        "-o", "--output", dest="outdir",
        help=f'Base directory to store processed daily output files (default: "processed-logs-json").'
    )
    ct_parser.add_argument(
        "-f", "--force", dest="force_outfile",
    ...
    ct_parser.add_argument(
        "--reduce-noise", dest="reduce_noise",
        action="store_true", default=False,
        help="Filter out noisy events."
    )
    ct_parser.add_argument(
        "-v", "--verbose", dest="verbose",
        action="store_true", help="Display progress and status information."
    )
    ...
    vf_parser.add_argument(
        "-r", "--read", dest="input", required=True,
        help="VPC Flow log file or directory to read."
    )
    vf_parser.add_argument(
        "-w", "--write", dest="outfile", required=True,
        help="Destination file to write parsed logs to."
    )
    vf_parser.add_argument(
        "-v", "--verbose", dest="verbose",
        action="store_true", help="Display progress and status information."
    ...
    # Kick Subcommand
    kick_parser = aws_subparsers.add_parser("kick", help="Append newline to JSON files if missing (fixes CloudTrail issues)")
    kick_parser.add_argument(
        "-t", "--target", dest="target", default=DEFAULT_DESTDIR_AWS,
...
def handle_cloudtrail(args: argparse.Namespace) -> None:
    ...
    if args.outdir and not args.outdir.startswith(DEFAULT_DESTDIR_AWS) and not args.force_outfile:
        sys.stderr.write(
            f'ERROR: Output location is not in {DEFAULT_DESTDIR_AWS}. Use "-f" to force.\n'
        )
        sys.exit(2)

    if (args.outdir and os.path.exists(args.outdir) and 
        args.outdir != DEFAULT_DESTDIR_AWS and not args.append):
        sys.stderr.write(
            f'ERROR: Output directory {args.outdir} already exists. Use "-a" to append.\n'
        )
        sys.exit(3)
...
def handle_vpcflow(args: argparse.Namespace) -> None:
    ...
    if not output_dir.startswith(DEFAULT_DESTDIR_NFARCH):
        if not args.outfile.startswith(DEFAULT_DESTDIR_NFARCH):
             sys.stderr.write(
                f"WARNING: Output file location {args.outfile} is not in {DEFAULT_DESTDIR_NFARCH}.\n"
            )
